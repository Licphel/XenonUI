...
public class OAL_TrackData : TrackData
{
    ...
    public static unsafe OAL_TrackData Read(FileHandle handle)
    {
        ReadOnlySpan<byte> file = File.ReadAllBytes(handle.Path);
        ...
        NativeManager.I0.Remind(() => { AL.DeleteBuffer(buffer); });
