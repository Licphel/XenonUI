    ...
    public Resolution(XGui curGui)
    {
        ...
        if(AllowResolution)
        {
            if(AllowResolution || curGui.ForceResolution)
            {
                ...
                while(Xsize / (Factor + 0.5f) >= Application.ScaledSize.x &&
                      Ysize / (Factor + 0.5f) >= Application.ScaledSize.y) Factor += 0.5f;

                if(LimInt && Factor * 2 % 2 != 0 && Factor - 0.5f > 0) Factor -= 0.5f;
            }
            else
            {
                Factor = 1;
