...
public class DefaultFontCarver : FontCarver
{
    ...
    public override void Draw(Graphics graphics, Font font, string text, float x, float y, float maxw)
    {
        ...
        var fontHeight = font.ScaledAndBlankedLineH;
        ...
        for(var i = 0; i < text.Length && i < MaxDrawableChars; i++)
        {
            var ch = text[i];
    ...
    public override void Draw(Graphics graphics, Font font, Lore text, float x, float y, float maxw)
    {
        ...
        foreach((LoreStyle, Func<string>) v in text.Content)
        {
            LoreStyle sp = v.Item1;
            var text0 = v.Item2();
            graphics.Color4(sp.Color);

            for(var i = 0; i < text0.Length && i < MaxDrawableChars; i++)
            {
                var ch = text0[i];
                Glyph g = font.GetGlyph(ch) * font.Scale;
                var w = g.Width;
        ...
        static void DrawOnce(Graphics graphics, ImageRegion map, LoreStyle sp, float drawX, float drawY, float w,
