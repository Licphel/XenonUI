...
public unsafe class GraphicsImageRGBA : Graphics
{
    ...
    public override void DrawImage(Image img, float x, float y, float width, float height, float srcX, float srcY, float srcWidth,
        float srcHeight)
    {
        ...
        fixed(byte* src0 = img.Data)
        {
            fixed(byte* dst_ = b)
            {
                byte* dst0 = dst_ + (ulong) (y * w * 4 + x * 4);
        
                for(int x1 = 0; x1 < width; x1++)
                {
                    for(int y1 = 0; y1 < height; y1++)
                    {
                        byte* src1 = src0 + (ulong) ((srcX + x1) * srcWidth * 4 + (srcY + y1) * 4);
                        byte* dst1 = dst0 + (ulong) (y1 * width * 4 + x1 * 4);
