...
public class NinePatch : Icon
{
	...
	public void Draw(Graphics graphics, float x, float y, float w, float h)
	{
		...
		{
			L.Draw(graphics, x, y + i * th, tw, th);//left draw
		}
		for(int i = 1; i < nw - 1; i++)
		{
			B.Draw(graphics, x + i * tw, y, tw, th);//bottom draw
		}
		RB.Draw(graphics, x2, y, tw, th);
		for(int i = 1; i < nh - 1; i++)
		{
			R.Draw(graphics, x2, y + i * th, tw, th);//right draw
		}
		LT.Draw(graphics, x, y2, tw, th);
		for(int i = 1; i < nw - 1; i++)
		{
			T.Draw(graphics, x + i * tw, y2, tw, th);//top draw
		}
