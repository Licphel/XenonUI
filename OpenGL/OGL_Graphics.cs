...
public unsafe class OGL_Graphics : Graphics
{
    ...
    {
        if(Verlen - (Vertices - Vertice0) < MinVertexBufSize) Flush();
    ...
    {
        for(var i = 0; i < arr.Length; i++) *Vertices++ = arr[i];
    }
    ...
    {
        Flush();
        var x0 = CameraNow.ToScrX(x, ViewportArray);
        var y0 = CameraNow.ToScrY(y, ViewportArray);
        var x1 = CameraNow.ToScrX(x + w, ViewportArray);
        var y1 = CameraNow.ToScrY(y + h, ViewportArray);
