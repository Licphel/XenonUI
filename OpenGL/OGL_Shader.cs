...
public class ShaderBuilds
{
    ...
    {
        var path = fileNoSuffix.Path;
        try
        {
            //if not using this way to read, it won't keep "\n", which shaders needs.
            var vert = StringIO.Read(new FileHandleImpl(path + ".vert"));
            var frag = StringIO.Read(new FileHandleImpl(path + ".frag"));
    ...
    public static OGL_Shader Build(string vertex, string fragment, Setup shaderSetup = null)
    {
        ...
        var vert = BuildShaderPart(vertex, ShaderType.VertexShader);
        var frag = BuildShaderPart(fragment, ShaderType.FragmentShader);
    ...
    private static int BuildShaderPart(string code, ShaderType type)
    {
        ...
        GL.GetShaderInfoLog(id, out var txt);
